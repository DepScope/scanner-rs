...
fn test_parse_pyproject_toml_fixture() {
    ...
    let parser = PyprojectTomlParser;
    let result = parser.parse(&content, Path::new("tests/fixtures/python/pyproject.toml")).unwrap();
    
    // Should have PEP 621 deps + Poetry deps (excluding python)
    assert!(result.len() >= 5);
    
    // Check PEP 621 dependencies
    assert!(result.iter().any(|d| d.name == "requests" && d.version == ">=2.28.0"));
    assert!(result.iter().any(|d| d.name == "numpy" && d.version == "==1.24.0"));
    
    // Check Poetry dependencies
    assert!(result.iter().any(|d| d.name == "django" && d.dep_type == DependencyType::Runtime));
    assert!(result.iter().any(|d| d.name == "pytest" && d.dep_type == DependencyType::Development));
}
...
"#;
    ...
    let result = parser.parse(content, Path::new("requirements.txt")).unwrap();
...
fn test_parse_requirements_txt_fixture() {
    ...
    let parser = RequirementsTxtParser;
    let result = parser.parse(&content, Path::new("tests/fixtures/python/requirements.txt")).unwrap();
    
    assert!(result.len() >= 5);
    assert!(result.iter().any(|d| d.name == "requests" && d.version == ">=2.28.0"));
    assert!(result.iter().any(|d| d.name == "numpy" && d.version == "==1.24.0"));
    assert!(result.iter().any(|d| d.name == "celery" && d.version == ">=5.3.0"));
