...
fn test_parse_poetry_lock_fixture() {
    ...
    let parser = PoetryLockParser;
    let result = parser.parse(&content, Path::new("tests/fixtures/python/poetry.lock")).unwrap();
    
    assert_eq!(result.len(), 3);
    assert!(result.iter().any(|d| d.name == "django" && d.version == "4.2.3"));
    assert!(result.iter().any(|d| d.name == "requests" && d.version == "2.31.0"));
    assert!(result.iter().any(|d| d.name == "pytest" && d.version == "7.4.0"));
}
...
fn test_parse_uv_lock_fixture() {
    ...
    let parser = UvLockParser;
    let result = parser.parse(&content, Path::new("tests/fixtures/python/uv.lock")).unwrap();
    
    assert_eq!(result.len(), 3);
    assert!(result.iter().any(|d| d.name == "flask" && d.version == "3.0.0"));
    assert!(result.iter().any(|d| d.name == "click" && d.version == "8.1.7"));
    assert!(result.iter().any(|d| d.name == "werkzeug" && d.version == "3.0.1"));
}
