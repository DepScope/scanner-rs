...
fn test_parse_fixture_file() {
    ...
    let parser = PackageJsonParser;
    let result = parser
        .parse(&content, Path::new("tests/fixtures/node/package.json"))
        .unwrap();
    ...
    assert!(result
        .iter()
        .any(|d| d.name == "typescript" && d.dep_type == DependencyType::Development));
    assert!(result
        .iter()
        .any(|d| d.name == "react-dom" && d.dep_type == DependencyType::Peer));
    assert!(result
        .iter()
        .any(|d| d.name == "fsevents" && d.dep_type == DependencyType::Optional));
}
